...
<div class="container py-5">
    <h1 class="text-primary mb-4">Edit Laporan Peminjaman Inventaris</h1>
        ...
        <div class="card-body">
            <form action="{{ route('mahasiswa.pelaporan.lapor_inventaris.update', $laporan->id_lapor_inventaris) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        @if($pelaporan->foto_awal)
                            <p class="mt-2">Foto saat ini: <a href="{{ Storage::url($laporan->foto_awal) }}" target="_blank" class="text-primary">Lihat</a></p>
                        @endif
                        @error('foto_awal')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        @if($laporan->foto_akhir)
                            <p class="mt-2">Foto saat ini: <a href="{{ Storage::url($laporan->foto_akhir) }}" target="_blank" class="text-primary">Lihat</a></p>
                        @endif
                        @error('foto_akhir')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control @error('deskripsi') is-invalid @enderror" id="deskripsi" name="deskripsi" rows="5">{{ old('deskripsi', $laporan->deskripsi) }}</textarea>
                    @error('deskripsi')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
