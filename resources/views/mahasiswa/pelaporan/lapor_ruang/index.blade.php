...
<div class="container py-5">
    ...
    @if($pelaporans->isEmpty())
        <div class="alert alert-info text-center">
            Belum ada laporan peminjaman ruangan.
        </div>
    @else
                        ...
                        <tbody>
                            @foreach($pelaporans as $index => $pelaporan)
                                <tr>
                                    ...
                                    <td class="p-3">{{ $pelaporan->datetime->format('d M Y, H:i') }}</td>
                                    <td class="p-3">{{ $pelaporan->oleh }}</td>
                                    <td class="p-3">{{ $pelaporan->kepada }}</td>
                                    <td class="p-3">{{ Str::limit($pelaporan->deskripsi, 50) }}</td>
                                    <td class="p-3">
                                        <a href="{{ route('pelaporans.show', $pelaporan->id_lapor_ruangan) }}" class="btn btn-sm btn-info me-1">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        <a href="{{ route('pelaporans.edit', $pelaporan->id_lapor_ruangan) }}" class="btn btn-sm btn-warning me-1">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <form action="{{ route('pelaporans.destroy', $pelaporan->id_lapor_ruangan) }}" method="POST" class="d-inline">
                                            @csrf
                                            @method('DELETE')
